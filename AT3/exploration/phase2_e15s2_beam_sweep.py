...
# %% [markdown]
# # E15-s2 beam-search hyperparameter sweep
#
# E15-s2 is the Phase 3 best at beam_width=5, length_penalty=0.7 (the
# Phase 2 universal default). Phase 1's E1 sweep showed beam decoding
# hyperparameters can move BLEU-4 by ±0.01, so worth checking whether
# the default is still optimal on a model from a different encoder.
#
# Try the natural grid:
#   * length_penalty ∈ {0.5, 0.7, 1.0}     (longer LP favours shorter caps)
#   * beam_width    ∈ {5, 10}              (wider beam = more diversity)
#
# Skip the (w=5, lp=0.7) cell since we already have it (test BLEU-4 0.403 / CIDEr 1.549).
# Five new settings to evaluate. ~5 min each.
...
OUT_ROOT = Path("output/phase2_results/phase2_e15s2_beam_sweep")
...
for s in SETTINGS:
    ...
    cfg = ExperimentConfig(
        run_name=f"e15s2_beam_sweep_{tag}",
